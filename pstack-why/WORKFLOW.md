...
Companion to the [pstack-how](../pstack-how/WORKFLOW.md) skill. [pstack-how](../pstack-how/WORKFLOW.md) answers what the code does and how it works. [pstack-why](WORKFLOW.md) answers what forces led to its shape.
...
Discover the tools actually exposed by the current harness, using its tool catalog or tool search. Inspect each selected schema. Do not assume a Cursor mcps directory or a named connector exists.
...
- Role: general-purpose worker, with the referenced prompt
- `model`: your configured why-investigators model (selected through owner policy)
- Access: read-only investigation with the required exposed tools. A missing tool is a capability gap; keep the harness permission mode.
...
1. **Source control investigator**. Git history, `gh` for PRs, code comments, tests. Cover it whenever the repository history is available. Best at surfacing *implementation-time rationale captured during review*.
...
- Role: general-purpose worker, with the referenced prompt
- `model`: your configured why-synthesizer model (selected through owner policy)
- Access: read-only investigation with the required exposed tools. A missing tool is a capability gap; keep the harness permission mode.
...
After the Sources Consulted block, if the user's [pstack-why](WORKFLOW.md) question is a precursor to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
