...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It delegates code changes only when permitted; without delegation it completes the work directly and reports the missing review separation. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the supported delegation API. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (only if the host and owner authorize nesting). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Local by default. Remote execution requires explicit authority and verified support. Keep machine-bound tasks local: `control-ui` or `control-cli` runtime verification (from `cursor-team-kit`). Privacy-aware session search. Simulators and local IDE state. Auth that exists only here. Cloud agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
A sub-coordinator brief adds its track boundary and unit list, its authorized spawn budget and execution environments, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (use the supported in-session event/wait mechanism), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Check resource requirements before restacking. Remote execution needs explicit authorization; if local capacity is insufficient, report that blocker.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the harness lifecycle status. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope. Network-drop, retry as-is. Tool-error, follow owner model-selection and fallback policy. Unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After a harness restart, re-read the standing orders and unit evidence, then inspect the host’s actual lifecycle state. Do not assume local runs died or remote runs survived. Resume only through supported lifecycle operations.
