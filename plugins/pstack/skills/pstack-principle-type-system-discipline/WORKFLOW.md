...
Applies to any typed language. Skills like [pstack-typescript-best-practices](../pstack-typescript-best-practices/WORKFLOW.md) ground it in specific syntax.
...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Have a parse function at every boundary that turns unstructured input into the typed model. See the [pstack-principle-boundary-discipline](../pstack-principle-boundary-discipline/WORKFLOW.md) principle skill for where to put validation.
