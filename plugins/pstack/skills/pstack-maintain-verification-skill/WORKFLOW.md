...
0. **Locate the target.** Inspect the project’s established source and discovery roots, including Owner Operator workspace skills when applicable. Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`). Several candidates → ask which one; none → stop and point at `/pstack-create-verification-skill` instead of inventing a target.
...
2. **Source wave.** One read-only source investigation per feature file, delegated concurrently only when supported and authorized. Otherwise do the same coverage directly and report that it was a single-agent source pass. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
