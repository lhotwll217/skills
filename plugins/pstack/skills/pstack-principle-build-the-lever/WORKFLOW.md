...
**Balance:** The bar is triviality, not repetition. A one-off still earns a lever when the lever is what makes the work checkable. Per the [Laziness Protocol](../pstack-principle-laziness-protocol/WORKFLOW.md), build the smallest script that does or proves the job, never a framework.

Distinct from [Encode Lessons in Structure](../pstack-principle-encode-lessons-in-structure/WORKFLOW.md), which makes a recurring instruction a durable guardrail. This is throughput and reviewability on the work in front of you. For scripting the verification itself, see [Prove It Works](../pstack-principle-prove-it-works/WORKFLOW.md).
