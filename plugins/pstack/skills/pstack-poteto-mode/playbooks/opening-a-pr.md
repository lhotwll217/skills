...
Apply only when the task includes preparing or opening a PR. Read-only requests and local installation do not authorize publication.

**Worktree.** Use an isolated worktree for concurrent writers. Preserve existing edits and branch state. Resolve conflicts within scope; destructive resets require explicit authority.

**Commits.** Commit liberally. Rebase into small, ordered commits before opening PRs. Each commit is a future PR: landable, ordered to tell the story. Amend when the fix belongs in a just-made commit. New commit when separable.

**PRs.** Use the deslop capability or manual substitute resolved in the portability entry point over the diff before commit. Run `/pstack-no-comments` before review. Write every PR title, PR description, and commit body with `/pstack-technical-writing`, then apply `/pstack-unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `pstack` or [pstack-poteto-mode](../WORKFLOW.md), as the scope. Keep the subject short and imperative. Name a real symbol when one carries the change. For example, `fix(pstack): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
A subagent that opens a PR follows [pstack-interrogate](../../pstack-interrogate/WORKFLOW.md), the resolved deslop capability or documented manual substitute, and [pstack-no-comments](../../pstack-no-comments/WORKFLOW.md), within its delegation permissions. It returns the URL and does not babysit. Return to the parent.
