...
Write each entry the way you'd tell a teammate what you did. Plain words, concrete actions, no AI speak or abstract jargon (the [pstack-unslop](../pstack-unslop/WORKFLOW.md) skill applies to log text too).
...
Place the log in the owner’s established project/workstream home outside product repositories, or in temporary scratch for an ephemeral run. Follow Chain of Context for new placement.

Keep internal trails out of product PRs unless intentionally shipped documentation is authorized.

## Rules

- One row is one decision or checkpoint.
- Append-only. A wrong call gets a new row that supersedes it. Never edit or delete history.
- Prefer evidence produced by committed scripts over hand-made one-offs (the [pstack-principle-encode-lessons-in-structure](../pstack-principle-encode-lessons-in-structure/WORKFLOW.md) principle skill).

## Audit the log against the transcript

At the end of the run, before handing back, check the log told the truth. Retrieve this run’s scoped transcript evidence through the approved session-search route, including tool calls and results. If unavailable, report the evidence gap. Walk the log against what actually happened:
