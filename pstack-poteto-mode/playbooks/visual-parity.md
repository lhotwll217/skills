...
3. Migrate one component at a time. Parallelize across worktrees, one owner per component (the [pstack-principle-separate-before-serializing-shared-state](../../pstack-principle-separate-before-serializing-shared-state/WORKFLOW.md) principle skill). Shared primitives migrate first as a blocking phase.
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. the available in-session wait mechanism per component until the diff is zero.
