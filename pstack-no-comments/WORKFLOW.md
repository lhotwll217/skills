...
1. Spawn the supported delegation API with the [Comment Sicko prompt](../pstack/agents/comment-sicko.md). Pass the scope. Do not restate its rules.
...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The [pstack-principle-fix-root-causes](../pstack-principle-fix-root-causes/WORKFLOW.md) and [pstack-principle-redesign-from-first-principles](../pstack-principle-redesign-from-first-principles/WORKFLOW.md) skills guide intent only. Neither authorizes widening the fence nor fixing instances outside it. Never bolt on symptom guards.
