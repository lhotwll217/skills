...
Your context lives in two records. Your own chat history holds what you did and decided. The shared record holds everything that happened around the same code under other names: the symptoms users keep reporting, the fixes that shipped and got reverted, the errors still firing in prod. That second record is what the [pstack-why](../pstack-why/WORKFLOW.md) skill searches, across source control, the issue tracker, chat and issue channels, long-form docs, and error tracking. A feature with a long bug tail keeps most of its story there, so don't reconstruct it from your transcripts alone.

Use the host’s privacy-aware session-search route described in the portability entry point. Retain stable session IDs and scoped evidence; do not derive transcript paths.

1. Classify, then route. One specific prior chat to resume is the `session-pickup` playbook, not this. Turning habits into a durable skill is [pstack-automate-me](../pstack-automate-me/WORKFLOW.md). A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
3. Search the bounded history through the approved session-search interface. Start with distinctive anchors, then inspect matching sessions by stable ID and bounded windows. Exclude the active/caller session where supported. Partition searches only when delegation is authorized; children use the same privacy policy. Return topic, user goal, decisions, open threads, corrections, and artifacts per session, citing its stable ID. Raw transcripts remain behind the search interface.
4. Sweep the shared record whenever the topic names a feature, file, subsystem, area, or bug. This is the default, not a judgment call, and "my work on X" does not exempt it. Hand it to the [pstack-why](../pstack-why/WORKFLOW.md) skill's source investigators, but steer their question from "why was this built this way" to "what's the current state, what's been tried and didn't hold, and what are users still reporting". Reuse its per-source playbooks, run the investigators in parallel with the chat-history mining, and inherit its posture: one investigator per source, null results are findings, skip an unavailable MCP and say so. Fold what comes back into the brief. Skip this step only for pure activity recall with no named target ("what did I do this week"), where your own history and live state are the entire answer.
5. Verify against live state. Take the PRs, branches, and tickets that the mining and the sweep surfaced and check them with `git` and `gh`. When the answer hinges on what an agent actually did (the tools it ran, files it read, errors it hit), retrieve the relevant calls and results through the approved session-search route with tool evidence enabled.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the [pstack-unslop](../pstack-unslop/WORKFLOW.md) skill, cite chat findings by UUID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
