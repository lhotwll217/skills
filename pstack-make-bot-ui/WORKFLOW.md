...
This workflow requires a host with webhook routines, secure secret-request UI, and an authorized network host. They are not supplied by this port. Before any action, confirm those capabilities and explicit authority to create a routine or expose a service. If any is missing, report the requirement and stop; Owner Operator schedules are not a webhook substitute. The host-specific recipe below applies only when its exact API and UI are exposed.
...
If Tailscale is absent or offline, report that prerequisite. Install or enroll it only under a separate explicit request using verified platform instructions.
