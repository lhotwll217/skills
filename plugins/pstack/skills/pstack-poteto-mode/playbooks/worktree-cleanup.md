...
3. Verify current ownership and usage through the approved session-search route and harness lifecycle. An unavailable history or status is unknown, not inactive. Confirm ownership before deletion.
4. Pause on irreversible loss. `wip:N` is N tracked uncommitted edits. Show the diff and get a decision first, since removing a clean worktree is recoverable from its branch but uncommitted work is gone. `scratch:N` is untracked data, not automatically disposable. Inspect and preserve it unless removal is authorized. Per Autonomy, clean and merged and not-in-use proceeds. `wip` and in-use pause.
